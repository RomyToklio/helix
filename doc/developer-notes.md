...
- [Development guidelines](#development-guidelines)
    - [General HELIX Core](#general-helix-core)
    - [Wallet](#wallet)
    - [General C++](#general-c)
...
HELIX Core uses [Doxygen](http://www.doxygen.nl/) to generate its official documentation.
...
HELIX Core is a multi-threaded application, and deadlocks or other
...
([`valgrind.supp`](https://github.com/projecthelixcoin/helix/blob/master/contrib/valgrind.supp))
which includes known Valgrind warnings in our dependencies that cannot be fixed
in-tree. Example use:

```shell
$ valgrind --suppressions=contrib/valgrind.supp src/test/test_helix
$ valgrind --suppressions=contrib/valgrind.supp --leak-check=full \
      --show-leak-kinds=all src/test/test_helix --log_level=test_suite
$ valgrind -v --leak-check=full src/helixd -printtoconsole
```
...
# A coverage report will now be accessible at `./test_helix.coverage/index.html`.
```
...
be sent upstream but bugfixes may also be prudent to PR against HELIX Core so that they can be integrated
...
file descriptors it will cause problems with HELIX's `select()` loop, because
...
- Try not to overload methods on argument type. E.g. don't make `getblock(true)` and `getblock("hash")`
  ...
  - *Rationale*: This is impossible to use with `helix-cli`, and can be surprising to users.
...
- Add every non-string RPC argument `(method, idx, name)` to the table `vRPCConvertParams` in `rpc/client.cpp`.
  ...
  - *Rationale*: `helix-cli` and the GUI debug console use this table to determine how to
